...
class CatalogService extends BaseService{
	...
	public function getPostsImages($postId, $w = 800, $h = 800){
		...
		if(sizeof($ids)){
			$q = "SELECT ID, post_name FROM wp_posts WHERE ID IN (".implode(",", $ids).")";
			$rows = $this->sql->select_rows($q);
			foreach($rows as $row){
				$obj = new stdClass();
				$obj->id = $row->ID;
				$obj->s = "/wp-content/uploads/auto/".$row->post_name."_200x200.jpg";
				$obj->m = "/wp-content/uploads/auto/".$row->post_name."_800x800.jpg";
				$obj->src = "/wp-content/uploads/auto/".$row->post_name.".jpg";
				$res[$row->ID] = $obj;
			}
		}
		return $res;
	...
	private function getImageMetaData($dir, $file, $ext, $file800x800, $file200x200){
		...
		$temp = array('width' => $sizeSource[0], 'height' => $sizeSource[1], 'dir' => $dir, 'file' => $dir."/".$file.".".$ext, 'sizes'=>array(
				'thumbnail' => array(
